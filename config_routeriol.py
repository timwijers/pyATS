...
class test_cases(aetest.Testcase):
    ...
    def check_ip_correct_routeriol1(self, uut):
        var = uut.parse('show ip interface brief ethernet0/1')
        var2 = uut.parse('show ip interface brief ethernet0/2')
        if var['interface']['Ethernet0/1']['ip_address'] != '192.168.2.2': self.failed('interface ethernet0/1 on '
                                                                                       'router IOL 1 does not have a '
                                                                                       'correct ip address')
        if var2['interface']['Ethernet0/2']['ip_address'] != '192.168.3.2': self.failed('interface ethernet0/2 on '
                                                                                        'router IOL 1 does not have a'
                                                                                        ' correct ip address')
    ...
    def check_ip_correct_routeriol2(self, uut2):
        var = uut2.parse('show ip interface brief ethernet0/1')
        var2 = uut2.parse('show ip interface brief ethernet0/2')
        if var['interface']['Ethernet0/1']['ip_address'] != '192.168.2.3': self.failed('interface ethernet0/1 on '
                                                                                       'router IOL 2 does not have a '
                                                                                       'correct ip address')
        if var2['interface']['Ethernet0/2']['ip_address'] != '192.168.3.3': self.failed('interface ethernet0/2 on '
                                                                                        'router IOL 2 does not have a'
                                                                                        ' correct ip address')
